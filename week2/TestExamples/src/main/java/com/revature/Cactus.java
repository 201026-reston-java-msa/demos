...
public class Cactus {
	...
	public int bloom() {
		System.out.println("The cactus has" + flowers);
		return flowers.size();
	}
	
	public double grow(double rainfall) {
		if (rainfall < 0) {
			throw new IllegalArgumentException("Railfall can't be negative");
		}
	...
	public Cactus seeding() {
		List<Flower> flowers2 = new ArrayList<Flower>(flowers);
		Collections.shuffle(flowers2);
		
		//Remove the right half of flowers
		for (int i=0; i<flowers.size() / 2; i++) {
			flowers2.remove(flowers2.size() -1);
		}
		
		return new Cactus(0.01 * height, 0.1 * waterStored, color, flowers2);
 	}
